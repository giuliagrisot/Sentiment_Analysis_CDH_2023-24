...
# PREP -----------
# if you erased your corpus_1, run this to recreate it:
...
corpus_token_SA <- readtext("TS_corpus_txt", encoding = "UTF-8")  %>%
  ...
  left_join(read_excel(
    # "D:/GitHub/zurich-sommerschule22.github.io/corpus_1/summer_school_corpus_info.xlsx"
...
# now we can add that to our corpus_1, too
...
# now we can see how many spatial entities match our corpus_1,
...
# we can also have a look at which places are more present in the corpus_1
...
# so we can create a dedicated corpus_1
...
# right now, with a tokenised corpus_1, we just get sentiment for spatial entiteis if these are in the sentiart list
...
# for the merging, we need our "long" corpus_1 with SA values aggregated by setniment
